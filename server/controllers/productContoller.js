...
export const createProduct = async (req,res) => {
  ...
  if (!title || !image || !rating || !rateCount || !price || !discountPrice) {
    res.status(400).json({ succes: false, errMsg: "all fields are required" });
    return;
  }
  try {
    ...
    res
      .status(200)
      .json({ succes: true, message: "product created successfully", product });
  } catch (error) {
...
export const allProducts = async (req,res) => {
  try {
    ...
    if (product && product.length === 0) {
      res
        .status(400)
        .json({ succes: false, errMsg: "no product(s) found / created" });
      return;
    }
